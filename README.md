...
In terms of comparing bias with training and test sets, if the model has a high bias, then the model fits the training set very well (i.e. low sums of squares of residuals). However, conversely, the model will not fit the test set well (i.e. this is because the model is close to being unbiased towards the training set).

<strong>Variance</strong> is the difference in fits between different datasets (i.e. test set vs. training set). It is important to note that if the model's bias is low (model fits the training set well), there is a trade-off, so model's variance will be high (model's sums of squares are high towards test set). Conversely, if model's bias is high, then the model's variance will be low, meaning that the model fits the test set well compared to the training set.
...
A <strong>simple model</strong> is a model that has <strong>high bias</strong> and <strong>low variance</strong>. On the other hand, a <strong>complex model</strong> is a model that has <strong>high variance</strong> and <strong>low bias</strong>. For example, OLS (Ordinary Least Squares) model will be a complex model with high variance and low bias. By applying regularization techniques, we are able to adjust our model to have lower variance and higher bias to get close to the sweet spot.
